...
func train(
	...
	var models = make([]*ModelNN, concurrency)
	models[0] = mainModel
...
func trainBatch(samples []Sample, models []*ModelNN) {
	var index int32 = -1
...
func applyGradients(models []*ModelNN) {
	for i := 1; i < len(models); i++ {
...
func calcAverageCost(samples []Sample, models []*ModelNN) float64 {
	var index int32 = -1
	var wg = &sync.WaitGroup{}

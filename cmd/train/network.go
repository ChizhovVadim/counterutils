...
func LoadNetwork(path string) Network {
	...
	net.Weights = make([]tuner.Matrix, len(topology.HiddenNeurons)+1)
	net.Biases = make([]tuner.Matrix, len(topology.HiddenNeurons)+1)
	...
	for i := 0; i < len(topology.HiddenNeurons)+1; i++ {
		...
		net.Weights[i] = tuner.Matrix{Data: data, Rows: outputSize, Cols: inputSize}
		...
		net.Biases[i] = tuner.Matrix{Data: data, Rows: outputSize, Cols: 1}
	}
	return net
}
